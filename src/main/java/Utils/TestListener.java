		...
		if (testConfigs != null)
			for (Config testConf : testConfigs) {
				if (testConf != null) {
					...
					testConf.logger.log(Status.FAIL, "Test Case Failed ---> " + result.getName());
		...
		if (testConfigs != null)
			for (Config testConf : testConfigs) {
				if (testConf != null) {
					...
					testConf.logger.log(Status.SKIP, message);
					TestBase.extent.flush();
				} else {
		...
		if (testConfigs != null)
			for (Config testConf : testConfigs) {
				if (testConf != null) {
					testConf.endTest(result);
					testConf.attachLogs(testConf.getTestName());
					testConf.logger.log(Status.PASS, testConf.getTestName());
					TestBase.extent.flush();
				} else {
